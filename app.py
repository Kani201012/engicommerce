...
# --- 1. APP CONFIGURATION ---
st.set_page_config(page_title="Kaydiem Titan v11.7 | Market Authority Build", layout="wide", page_icon="💎")
...
st.title("🏗️ Kaydiem Titan Supreme v11.7")
...
if st.button("🚀 DEPLOY 100% MARKET-DOMINATING ASSET"):
    ...
    # Logo HTML
    logo_final = f'<img src="{biz_logo}" alt="{biz_name}" class="h-10 md:h-16 w-auto object-contain">' if biz_logo else f'<span class="text-xl md:text-3xl font-black tracking-tighter uppercase" style="color:var(--p)">{biz_name}</span>'
    ...
    * {{ box-sizing: border-box; }}
    ...
    def get_layout(title_tag, desc_tag, body_html, is_h=False):
        ...
        # --- RECTIFIED DYNAMIC INJECTION SCRIPT (v11.7) ---
        dynamic_script = ""
            ...
            async function fetchLiveData() {{
                try {{
                    ...
                    rows.forEach((line, idx) => {{
                        // Regex to handle commas inside quotes (Excel standard)
                        const parts = line.match(/(".*?"|[^",\\s]+)(?=\\s*,|\\s*$)/g);
                        if (parts && parts.length >= 2) {{
                            const cleanParts = parts.map(p => p.replace(/^"|"$/g, "").trim());
                            const p = {{ 
                                id: idx, name: cleanParts[0], price: cleanParts[1], desc: cleanParts[2] || "", 
                                img1: cleanParts[3] || "{img_f}", img2: cleanParts[4] || "", img3: cleanParts[5] || "" 
                            }};
...
<body class="bg-white">
    <nav class="glass-nav p-4 md:p-6 shadow-sm">
        <div class="max-w-[1440px] mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
            <a href="index.html" class="no-underline">{logo_final}</a>
    ...
    <footer class="bg-slate-950 text-slate-400 py-24 px-10 border-t border-slate-900">
        <div class="max-w-[1440px] mx-auto grid md:grid-cols-3 gap-16">
            <div>
                {logo_final.replace('h-10 md:h-16', 'h-8 mb-6 opacity-70')}
            ...
            <div class="md:text-right">
                <h4 class="text-white font-bold mb-8 uppercase text-xs text-brand tracking-widest" style="color:var(--s)">Direct Connect</h4>
                <p class="text-lg font-bold text-white leading-loose underline decoration-blue-600 decoration-4 underline-offset-8 uppercase tracking-widest">Support Hub</p>
    ...
    <section id="inventory" class="py-32 px-6 max-w-[1440px] mx-auto text-center border-b">
        <h2 class="section-title mb-20 uppercase tracking-tighter" style="color:var(--p)">Exclusive Collection</h2>
    ...
    st.success("💎 TITAN SUPREME v11.7 FINAL PLATINUM DEPLOYED. Footer links and products rectified.")
    st.download_button("📥 DOWNLOAD COMPLETE BIZ PACKAGE", z_b.getvalue(), f"{biz_name.lower().replace(' ', '_')}_v11_7.zip")
