...
# --- 1. APP CONFIGURATION ---
st.set_page_config(page_title="Kaydiem Titan v11.8 | Luxury Showroom Architect", layout="wide", page_icon="💎")
...
st.title("🏗️ Kaydiem Titan Supreme v11.8")
...
with tabs[3]:
    ...
    st.error("MANDATORY: Use Pipe (|) in your Google Sheet. Format: Name | Price | Description | Img1 | Img2 | Img3")
...
# --- 3. THE SUPREME ENGINE V11.8 ---

if st.button("🚀 DEPLOY 100% LUXURY DYNAMIC ASSET"):
    ...
    logo_h = f'<img src="{biz_logo}" alt="{biz_name}" class="h-10 md:h-16 w-auto object-contain">' if biz_logo else f'<span class="text-xl md:text-3xl font-black tracking-tighter uppercase" style="color:var(--p)">{biz_name}</span>'
    ...
    * {{ box-sizing: border-box; }}
    ...
    .btn-p {{ background: var(--p); color: white !important; padding: 0.9rem 2.2rem; border-radius: var(--radius); font-weight: 900; transition: all 0.4s; display: inline-block; text-align: center; border:none; text-decoration:none; text-transform: uppercase; font-size: 11px; letter-spacing: 0.15em; }}
    ...
    .hero-mask {{ background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.5)), url('{img_h}'); background-size: cover; background-position: center; min-height: 85vh; display: flex; align-items: center; justify-content: center; width: 100%; margin: 0; padding: 140px 20px 60px 20px; }}
    ...
    .thumb-btn {{ cursor: pointer; border: 2px solid transparent; transition: 0.3s; }}
    .thumb-btn:hover {{ border-color: var(--s); transform: scale(1.05); }}
    ...
    def get_layout(title_tag, desc_tag, body_html, is_h=False):
        ...
        # --- MASTER DYNAMIC SHOWROOM SCRIPT (v11.8) ---
        dyn_script = ""
        if is_h and sheet_url:
            ...
            async function fetchLiveData() {{
                try {{
                    ...
                    // RECTIFIED: Split by Pipe (|) to preserve commas in price/text
                        ...
                        if (parts.length >= 2) {{
                            ...
                            <div onclick="openProduct(${{idx}})" class="product-card flex flex-col justify-between transition-all hover:scale-[1.03]">
                                ...
                                <div>
                                    ...
                                    <p class="text-slate-400 text-[10px] font-black uppercase tracking-widest italic underline underline-offset-4">Open Showroom →</p>
                                </div>
                            </div>`;
                        }
                    }});
            ...
            function openProduct(id) {{
                ...
                // Show/Hide Thumbs based on availability
...
<body class="bg-white">
        ...
        <div class="modal-content shadow-2xl animate-in zoom-in duration-300">
            <div class="grid md:grid-cols-2">
                ...
                <div class="p-12 flex flex-col justify-center text-left">
                    ...
                    <button onclick="closeModal()" class="text-xs font-black uppercase tracking-widest opacity-30 mt-8 underline no-underline">Return to List</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <footer class="bg-slate-950 text-slate-400 py-24 px-10 border-t border-slate-900">
        <div class="max-w-[1440px] mx-auto grid md:grid-cols-3 gap-16 text-left">
            <div>
                {logo_h.replace('h-10 md:h-16', 'h-8 mb-6 opacity-70')}
            ...
            <div>
                <h4 class="text-white font-bold mb-8 uppercase text-xs tracking-widest">Company Hub</h4>
            ...
            <div class="md:text-right">
                <h4 class="text-white font-bold mb-8 uppercase text-xs text-brand tracking-widest underline decoration-blue-600 decoration-4 underline-offset-8" style="color:var(--s)">Direct Connect</h4>
    ...
    <section class="hero-mask px-6 text-center text-white">
        <div class="max-w-[1200px] mx-auto">
            ...
            <a href="#showroom" class="btn-accent uppercase tracking-[0.4em] text-[10px] md:text-sm shadow-2xl">Explore Showroom</a>
        </div>
    </section>
    
    <section id="showroom" class="py-32 px-6 max-w-[1440px] mx-auto text-center border-b">
        <h2 class="section-title mb-20 uppercase tracking-tighter" style="color:var(--p)">Curated Collection</h2>
        <div id="live-data-container" class="grid grid-cols-1 md:grid-cols-4 gap-10 text-left">
            <p class="p-20 text-center text-slate-400 font-bold animate-pulse uppercase tracking-widest">Establishing Connection to Showroom Hub...</p>
        </div>
    </section>

    <section class="bg-slate-50 py-32 px-6 border-y text-center">
        <div class="max-w-4xl mx-auto">
            <h2 class="text-5xl font-black mb-12 uppercase tracking-tighter text-p" style="color:var(--p)">Industry Heritage</h2>
            <p class="text-2xl text-slate-600 mb-12 italic leading-relaxed">"Delivering high-end technical and aesthetic assets for New Delhi's finest celebrations. Quality verified by engineering precision."</p>
    ...
    with zipfile.ZipFile(z_b, "a", zipfile.ZIP_DEFLATED, False) as z_f:
        ...
        z_f.writestr("about.html", get_layout("About", "History", f"<section class='max-w-7xl mx-auto py-40 px-6'><h1 class='legal-bold-title uppercase tracking-tighter text-brand' style='color:var(--p)'>About Our Heritage</h1><div class='text-xl md:text-2xl leading-relaxed text-slate-700 legal-text'>{about_txt}</div><img src='{img_g}' class='mt-20 w-full h-[600px] object-cover shadow-2xl' style='border-radius: var(--radius)'></section>"))
        z_f.writestr("contact.html", get_layout("Contact", "Location", f"<section class='max-w-[1440px] mx-auto py-40 px-6 text-center'><h1 class='legal-bold-title uppercase tracking-tighter text-brand' style='color:var(--p)'>Connect</h1><div class='grid md:grid-cols-2 gap-16 text-left'><div class='bg-slate-950 p-12 md:p-24 text-white' style='border-radius: var(--radius)'><p class='text-4xl font-black mb-8 text-white'>{biz_phone}</p><p class='text-2xl mb-12 opacity-80'>{biz_addr}</p><a href='tel:{biz_phone}' class='btn-accent w-full'>BOOK CONSULTATION</a></div><div class='rounded-[3rem] overflow-hidden border shadow-2xl bg-slate-100' style='min-height:350px'>{map_iframe}</div></div></section>"))
        z_f.writestr("privacy.html", get_layout("Privacy", "Legal", f"<div class='max-w-4xl mx-auto py-40 px-6'><h1 class='legal-bold-title uppercase tracking-tighter text-brand' style='color:var(--p)'>Privacy Policy</h1><div class='text-lg legal-text'>{priv_body}</div></div>"))
        z_f.writestr("terms.html", get_layout("Terms", "Legal", f"<div class='max-w-4xl mx-auto py-40 px-6'><h1 class='legal-bold-title uppercase tracking-tighter text-brand' style='color:var(--p)'>Terms & Conditions</h1><div class='text-lg legal-text'>{terms_body}</div></div>"))
    ...
    st.success("💎 TITAN SUPREME v11.8 DEPLOYED. Luxury Gallery & Pipe Parser active.")
    st.download_button("📥 DOWNLOAD PLATINUM BIZ PACKAGE", z_b.getvalue(), f"{biz_name.lower().replace(' ', '_')}_v11_8.zip")
