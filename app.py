...
# --- 1. APP CONFIGURATION ---
st.set_page_config(page_title="Kaydiem Titan v10.2 | Platinum Web Architect", layout="wide", page_icon="💎")
...
st.title("🏗️ Kaydiem Titan Supreme v10.2")
...
with tabs[1]:
    ...
    seo_d = st.text_input("Meta Description (160 Chars)", "Best luxury wedding planners in New Delhi.")
    biz_key = st.text_input("SEO Keywords", "Wedding Decor, Luxury Planners")
...
with tabs[4]:
    testi_data = st.text_area("Testimonials (Name | Quote)")
    faq_data = st.text_area("FAQ (Question? ? Answer)")
...
if st.button("🚀 DEPLOY FLAWLESS 10.2 ASSET"):
    ...
    logo_html = f'<img src="{biz_logo}" alt="{biz_name}" class="h-10 md:h-16 w-auto object-contain mx-auto md:mx-0">' if biz_logo else f'<span class="text-xl md:text-3xl font-black tracking-tighter uppercase" style="color:var(--p)">{biz_name}</span>'
    ...
    * {{ box-sizing: border-box; }}
    ...
    .hero-title {{ font-size: clamp(1.2rem, 8vw, 95px); text-shadow: 0 4px 20px rgba(0,0,0,0.4); line-height: 1; }}
    ...
    /* RECTIFIED HERO - NO GAP, NO OVERLAP */
    .hero-mask {{ 
...
<body class="bg-white">
    ...
    <footer class="bg-slate-950 text-slate-400 py-24 px-10 border-t border-slate-900">
        <div class="max-w-[1440px] mx-auto grid md:grid-cols-4 gap-16 text-left">
            ...
            <div><h4 class="text-white font-bold mb-8 uppercase text-xs text-brand tracking-widest" style="color:var(--s)">Direct Connect</h4><p class="text-lg font-bold text-white leading-loose underline decoration-blue-600 decoration-4 underline-offset-8 uppercase tracking-widest">Technical Support</p><p class="text-xl mt-4" style="white-space:nowrap; color:white;">{biz_phone}</p><p class="text-xs mt-2">{biz_email}</p></div>
        </div>
    </footer>
    {dyn_script}
</body></html>"""
    ...
    serv_html = "".join([f'<div class="bg-slate-50 p-12 rounded-[2.5rem] border border-slate-100 shadow-xl hover:scale-[1.02] transition-transform"><h3 class="text-2xl font-black mb-4 uppercase" style="color:var(--p)">{s.strip()}</h3><p class="text-slate-500 text-xs font-bold uppercase tracking-tight italic text-left">Verified technical solution for {biz_name}.</p></div>' for s in biz_serv.splitlines() if s.strip()])
    t_cards = "".join([f'<div class="p-10 bg-slate-50 rounded-[3rem] border border-slate-100 italic text-xl shadow-inner mb-8" style="color:var(--p)">"{t.split("|")[1].strip()}"<br><span class="font-black not-italic text-sm block mt-6 uppercase tracking-widest text-brand" style="color:var(--p)">— {t.split("|")[0].strip()} <span class="text-emerald-500 font-black ml-2 text-xs">● Verified Partner</span></span></div>' for t in testi_data.splitlines() if "|" in t])
    f_cards = "".join([f'<details class="mb-6 bg-white p-6 rounded-2xl border border-slate-100 cursor-pointer shadow-sm"><summary class="font-black text-lg uppercase tracking-tight">{f.split("?")[0].strip()}?</summary><p class="mt-4 text-slate-600 leading-relaxed font-medium text-sm">{f.split("?")[1].strip()}</p></details>' for f in faq_data.splitlines() if "?" in f])
    ...
    with zipfile.ZipFile(z_b, "a", zipfile.ZIP_DEFLATED, False) as z_f:
        ...
        z_f.writestr("contact.html", get_layout("Contact", "Location", f"<section class='max-w-[1440px] mx-auto py-32 px-6 text-center'><h1 class='legal-bold-title uppercase tracking-tighter'>Technical Hub</h1><div class='grid md:grid-cols-2 gap-16 text-left'><div class='bg-slate-950 p-12 md:p-24 text-white' style='border-radius: var(--radius)'><p class='text-4xl font-black mb-8 text-white'>{biz_phone}</p><p class='text-2xl mb-12 opacity-80'>{biz_addr}</p><a href='tel:{biz_phone}' class='btn-accent w-full'>BOOK CONSULTATION</a></div><div class='rounded-[3rem] overflow-hidden border shadow-2xl bg-slate-100' style='min-height:300px'>{map_iframe}</div></div></section>"))
    ...
    st.success("💎 TITAN SUPREME v10.2 DEPLOYED. Fixed all mobile scaling and variable errors.")
    st.download_button("📥 DOWNLOAD COMPLETE PACKAGE", z_b.getvalue(), f"{biz_name.lower().replace(' ', '_')}_v10_2.zip")
