...
# --- 1. APP CONFIGURATION ---
st.set_page_config(page_title="Kaydiem Titan v11.6 | Platinum Web Architect", layout="wide", page_icon="💎")
...
st.title("🏗️ Kaydiem Titan Supreme v11.6")
...
if st.button("🚀 DEPLOY FLAWLESS PLATINUM ASSET"):
    ...
    * {{ box-sizing: border-box; }}
    ...
    /* RECTIFIED PROFESSIONAL BUTTON UI */
    ...
    def get_layout(title_tag, desc_tag, body_html, is_h=False):
        ...
        # --- RECTIFIED DYNAMIC E-COM SCRIPT (v11.6) ---
        dynamic_script = ""
...
<body class="bg-white flex flex-col min-h-screen">
    ...
    <footer class="bg-slate-950 text-slate-400 py-24 px-10 border-t border-slate-900">
        <div class="max-w-[1440px] mx-auto grid md:grid-cols-2 gap-16">
            ...
            <div class="md:text-right">
                <h4 class="text-white font-bold mb-4 uppercase text-xs tracking-widest tracking-widest underline decoration-blue-600 decoration-4 underline-offset-8">Support Hub</h4>
    ...
    dynamic_section = f"""<section id="inventory" class="py-32 px-6 max-w-[1440px] mx-auto text-center border-b"><h2 class="section-title mb-20 uppercase tracking-tighter">Exclusive Collection</h2><div id="live-data-container" class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-8 text-left"><p class="p-20 text-center text-slate-400 font-bold animate-pulse uppercase tracking-widest">Opening Data Hub...</p></div></section>""" if sheet_url else ""
    ...
    <section class="hero-mask px-6 text-center text-white">
        <div class="max-w-[1200px] mx-auto">
            ...
            <a href="#inventory" class="btn-accent uppercase tracking-[0.4em] text-[10px] md:text-sm shadow-2xl">Consult Now</a>
        </div>
    </section>
    ...
    with zipfile.ZipFile(z_b, "a", zipfile.ZIP_DEFLATED, False) as z_f:
        ...
        z_f.writestr("about.html", get_layout("About", "History", f"<section class='max-w-7xl mx-auto py-32 px-6'><h1 class='legal-bold-title uppercase tracking-tighter'>About Our Heritage</h1><div class='text-xl md:text-2xl leading-relaxed text-slate-700 legal-text'>{about_txt}</div><img src='{img_g}' class='mt-20 w-full h-[600px] object-cover shadow-2xl' style='border-radius: var(--radius)'></section>"))
    ...
    st.success("💎 TITAN SUPREME v11.6 PLATINUM BUILD DEPLOYED. UI and E-commerce fixed.")
    st.download_button("📥 DOWNLOAD COMPLETE BIZ PACKAGE", z_b.getvalue(), f"{biz_name.lower().replace(' ', '_')}_v11_6.zip")
