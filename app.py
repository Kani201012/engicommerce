...
# --- 1. APP CONFIGURATION ---
st.set_page_config(page_title="Kaydiem Titan v11.9 | Full-Stack Stable Architect", layout="wide", page_icon="💎")
...
st.title("🏗️ Kaydiem Titan Supreme v11.9")
...
# --- 3. THE SUPREME ENGINE V11.9 ---

if st.button("🚀 DEPLOY 100% STABLE DYNAMIC ASSET"):
    ...
    logo_h = f'<img src="{biz_logo}" alt="{biz_name}" class="h-10 md:h-16 w-auto object-contain">' if biz_logo else f'<span class="text-xl md:text-3xl font-black uppercase" style="color:var(--p)">{biz_name}</span>'
    ...
    # RECTIFIED: Doubled curly braces for CSS
    theme_css = f"""
    ...
    * {{ box-sizing: border-box; }}
    ...
    def get_layout(title_tag, desc_tag, body_html, is_h=False):
        ...
        # RECTIFIED: Doubled curly braces for JS logic inside f-string
        dyn_script = ""
        if is_h and sheet_url:
                        ...
                        if (parts.length >= 2) {{
                            ...
                            <div onclick="openProduct(${{idx}})" class="product-card flex flex-col justify-between transition-all hover:scale-[1.03]">
                                ...
                                <div>
                                    ...
                                    <p class="text-slate-400 text-[10px] font-black uppercase tracking-widest italic underline decoration-slate-100 underline-offset-4">Open Details →</p>
                                </div>
                            </div>`;
                        }}
                    }});
            ...
            function openProduct(id) {{
                ...
                `;
                document.getElementById('m-wa').href = "{wa_base_url}" + encodeURIComponent("Interest: " + p.name + " (" + p.price + ")");
...
<body class="bg-white">
        ...
        <div class="modal-content shadow-2xl animate-in zoom-in duration-300">
            <div class="grid md:grid-cols-2">
                ...
                <div class="p-12 flex flex-col justify-center text-left">
                    ...
                    <a id="m-wa" href="#" target="_blank" class="btn-accent w-full uppercase tracking-widest">Book Selection</a>
    ...
    <footer class="bg-slate-950 text-slate-400 py-24 px-10 border-t border-slate-900">
        <div class="max-w-[1440px] mx-auto grid md:grid-cols-3 gap-16 text-left">
            ...
            <div class="md:text-right">
                <h4 class="text-white font-bold mb-8 uppercase text-xs tracking-widest underline decoration-blue-600 decoration-4 underline-offset-8" style="color:var(--s)">Direct Connect</h4>
    ...
    <section id="showroom" class="py-32 px-6 max-w-[1440px] mx-auto text-center border-b">
        <h2 class="section-title mb-20 uppercase tracking-tighter" style="color:var(--p)">Exclusive Offers</h2>
    ...
    with zipfile.ZipFile(z_b, "a", zipfile.ZIP_DEFLATED, False) as z_f:
        ...
        z_f.writestr("about.html", get_layout("About", "History", f"<section class='max-w-7xl mx-auto py-40 px-6'><h1 class='legal-bold-title uppercase tracking-tighter text-brand' style='color:var(--p)'>Our Heritage</h1><div class='text-xl md:text-2xl leading-relaxed text-slate-700 legal-text'>{about_txt}</div><img src='{img_g}' class='mt-20 w-full h-[600px] object-cover shadow-2xl' style='border-radius: var(--radius)'></section>"))
        z_f.writestr("contact.html", get_layout("Contact", "Location", f"<section class='max-w-[1440px] mx-auto py-40 px-6 text-center'><h1 class='legal-bold-title uppercase tracking-tighter text-brand' style='color:var(--p)'>Connect</h1><div class='grid md:grid-cols-2 gap-16 text-left'><div class='bg-slate-950 p-12 md:p-24 text-white' style='border-radius: var(--radius)'><p class='text-4xl font-black mb-8 text-white'>{biz_phone}</p><p class='text-2xl mb-12 opacity-80'>{biz_addr}</p><a href='tel:{biz_phone}' class='btn-accent w-full no-underline uppercase tracking-widest font-black'>Book Consultation</a></div><div class='rounded-[3rem] overflow-hidden border shadow-2xl bg-slate-100' style='min-height:350px'>{map_iframe}</div></div></section>"))
    ...
    st.success("💎 TITAN SUPREME v11.9 FINAL RECTIFIED BUILD DEPLOYED.")
    st.download_button("📥 DOWNLOAD PLATINUM PACKAGE", z_b.getvalue(), f"{biz_name.lower().replace(' ', '_')}_v11_9.zip")
