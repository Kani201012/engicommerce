...
# --- 1. APP CONFIGURATION ---
st.set_page_config(page_title="Kaydiem Titan v10.1 | Pixel-Perfect Architect", layout="wide", page_icon="💎")
...
st.title("🏗️ Kaydiem Titan Supreme v10.1")
...
if st.button("🚀 DEPLOY PIXEL-PERFECT ASSET"):
    ...
    wa_final_url = f"https://wa.me/{wa_c}?text=Hello%20{biz_name.replace(' ', '%20')}"
    ...
    * {{ box-sizing: border-box; }}
    ...
    h1, h2, h3 {{ font-family: '{h_font}', sans-serif; font-weight: {h_weight}; letter-spacing: {ls}; text-transform: uppercase; line-height: 1.1; word-break: normal; hyphens: none; }}
    
    /* RECTIFIED TYPOGRAPHY SCALING */
    ...
    .product-card-title {{ font-size: clamp(1rem, 4vw, 1.5rem); line-height: 1.2; }}
    
    /* FOOTER TEXT SIZE CONTROL */
    .footer-text {{ font-size: 0.85rem; line-height: 1.5; color: #94a3b8; }}
    .phone-link {{ white-space: nowrap; font-weight: 700; color: white; }}
    
    .btn-supreme {{ background: var(--s); color: white; padding: 1.1rem 2.5rem; border-radius: var(--radius); font-weight: 900; transition: all 0.4s; display: inline-block; text-align: center; border:none; box-shadow: 0 10px 20px -5px var(--s); text-decoration:none; cursor: pointer; }}
    ...
    def get_layout(t_tag, d_tag, b_html, is_h=False):
        ...
        if is_h and sheet_url:
            ...
            async function fetchLiveData() {{
                try {{
                    ...
                    if (csv.trim().startsWith("<!DOCTYPE") || csv.trim().startsWith("<html")) {{
                        document.getElementById('live-data-container').innerHTML = "<div class='col-span-full p-20 text-center text-red-600 font-bold'>CSV LINK ERROR.</div>";
                        return;
                    }}
                    ...
                    rows.forEach((parts, idx) => {{
                        if (parts.length >= 2) {{
                            ...
                            <div onclick="openProduct(${{idx}})" class="product-card flex flex-col justify-between transition-all hover:scale-[1.03]">
                                ...
                                <div>
                                    <h3 class="product-card-title font-black mb-2 uppercase" style="color:var(--p)">${{p.name}}</h3>
            ...
            function openProduct(id) {{
                ...
                document.getElementById('m-wa').href = "https://wa.me/{wa_c}?text=" + encodeURIComponent("I'm interested in " + p.name);
...
<head>
    ...
    {v_tag}
    <title>{t_tag} | {biz_name}</title>
    <meta name="description" content="{d_tag}"><meta name="keywords" content="{biz_key}">
...
<body class="bg-white">
    ...
    <main class="flex-grow pt-24 md:pt-0">{b_html}</main>
        ...
        <div class="modal-content shadow-2xl animate-in zoom-in duration-300">
            <div class="grid md:grid-cols-2">
                <div class="p-6 bg-slate-50">
                    <img id="m-img-1" class="w-full h-80 object-cover rounded-[2rem] shadow-xl">
                </div>
    ...
    <footer class="bg-slate-950 text-slate-400 py-24 px-10 border-t border-slate-900">
        <div class="max-w-[1440px] mx-auto grid md:grid-cols-4 gap-16 text-left">
            <div class="col-span-2">
                ...
                <p class="footer-text mb-10 max-w-md">{biz_addr}</p>
            ...
            <div><h4 class="text-white font-bold mb-8 uppercase text-xs text-brand tracking-widest" style="color:var(--s)">Contact Hub</h4><p class="footer-text font-bold text-white leading-loose underline decoration-blue-600 decoration-4 underline-offset-8 uppercase tracking-widest">Direct Support</p><p class="phone-link text-xl mt-4">{biz_phone}</p><p class="text-xs mt-2">{biz_email}</p></div>
        </div>
    </footer>
    {d_script}
</body></html>"""
    ...
    # --- INDEX CONTENT ---
    s_h = "".join([f'<div class="bg-slate-50 p-12 rounded-[2.5rem] border border-slate-100 shadow-xl hover:scale-[1.02] transition-transform"><h3 class="product-card-title font-black mb-4 uppercase text-brand" style="color:var(--p)">{s.strip()}</h3><p class="text-slate-500 text-xs font-bold uppercase tracking-tight italic text-left">Verified technical solution for {biz_name}.</p></div>' for s in biz_serv.splitlines() if s.strip()])
    t_h = "".join([f'<div class="p-10 bg-slate-50 rounded-[3rem] border border-slate-100 italic text-xl shadow-inner mb-8 text-p" style="color:var(--p)">"{t.split("|")[1].strip()}"<br><span class="font-black not-italic text-sm block mt-6 uppercase tracking-widest text-brand" style="color:var(--p)">— {t.split("|")[0].strip()} <span class="text-emerald-500 font-black ml-2 text-xs">● Verified Partner</span></span></div>' for t in testi_txt.splitlines() if "|" in t])
    f_h = "".join([f'<details class="mb-6 bg-white p-6 rounded-2xl border border-slate-100 cursor-pointer shadow-sm"><summary class="font-black text-lg uppercase tracking-tight">{f.split("?")[0].strip()}?</summary><p class="mt-4 text-slate-600 leading-relaxed font-medium text-sm">{f.split("?")[1].strip()}</p></details>' for f in faq_txt.splitlines() if "?" in f])

    d_s = f"""<section class="py-32 px-6 max-w-[1440px] mx-auto text-center border-b"><h2 class="section-title mb-20 uppercase tracking-tighter" style="color:var(--p)">Verified Packages</h2><div id="live-data-container" class="grid grid-cols-1 md:grid-cols-4 gap-10 text-left"><p class="p-20 text-center text-slate-400 font-bold animate-pulse uppercase tracking-widest">Connecting to Data Hub...</p></div></section>""" if sheet_url else ""
    ...
    <section class="bg-slate-50 py-32 px-6 border-y text-left">
        <div class="max-w-[1440px] mx-auto grid md:grid-cols-2 gap-24 items-center">
            ...
            <div>
                ...
                <a href="about.html" class="btn-supreme text-xs tracking-widest uppercase">View Our Story</a>
            </div>
        </div>
    </section>
    <section class="py-32 px-6 max-w-7xl mx-auto text-center"><div class="grid md:grid-cols-2 gap-24 text-left"><div><h2 class="text-4xl font-black mb-16 uppercase tracking-tighter" style="color:var(--p)">Partner Success</h2>{t_h}</div><div><h2 class="text-4xl font-black mb-16 uppercase tracking-tighter" style="color:var(--p)">Expert Insights</h2>{f_h}</div></div></section>
    """
    ...
    with zipfile.ZipFile(z_b, "a", zipfile.ZIP_DEFLATED, False) as z_f:
        ...
        z_f.writestr("contact.html", get_layout("Contact", "Location", f"<section class='max-w-[1440px] mx-auto py-32 px-6 text-center'><h1 class='legal-bold-title uppercase tracking-tighter'>Connect with Us</h1><div class='grid md:grid-cols-2 gap-16 text-left'><div class='bg-slate-950 p-12 md:p-24 text-white' style='border-radius: var(--radius)'><p class='text-4xl font-black mb-8 text-white'>{biz_phone}</p><p class='text-2xl mb-12 opacity-80'>{biz_addr}</p><a href='tel:{biz_phone}' class='btn-accent w-full'>BOOK CONSULTATION</a></div><div class='rounded-[3rem] overflow-hidden border shadow-2xl bg-slate-100' style='min-height:300px'>{map_iframe}</div></div></section>"))
    ...
    st.success("💎 TITAN SUPREME v10.1 DEPLOYED. Fixed all mobile breakages.")
    st.download_button("📥 DOWNLOAD PIXEL-PERFECT PACKAGE", z_b.getvalue(), f"{biz_name.lower().replace(' ', '_')}_v10_1.zip")
