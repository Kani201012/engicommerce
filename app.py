...
# --- 1. APP CONFIGURATION ---
st.set_page_config(page_title="Kaydiem Titan v11.2 | Platinum Architect", layout="wide", page_icon="💎")
...
st.title("🏗️ Kaydiem Titan Supreme v11.2")
...
if st.button("🚀 DEPLOY FLAWLESS ENTERPRISE ASSET"):
    ...
    * {{ box-sizing: border-box; }}
    ...
    .hero-title {{ font-size: clamp(2.2rem, 9vw, 110px); text-shadow: 0 4px 20px rgba(0,0,0,0.4); line-height: 1; }}
    .section-title {{ font-size: clamp(1.8rem, 6vw, 75px); color: var(--p); }}
    
    .btn-supreme {{ background: var(--p); color: white; padding: 1.1rem 2.8rem; border-radius: var(--radius); font-weight: 900; transition: all 0.4s; display: inline-block; text-align: center; border:none; }}
    .btn-accent {{ background: var(--s); color: white; padding: 1rem 2.5rem; border-radius: var(--radius); font-weight: 900; transition: all 0.4s; display: inline-block; text-align: center; border:none; box-shadow: 0 10px 20px -5px var(--s); text-decoration:none; cursor: pointer; }}
                        ...
                        if (parts.length >= 2) {{
                            ...
                            <div onclick="openProduct(${{idx}})" class="product-card flex flex-col justify-between transition-all hover:scale-[1.03]">
                                ...
                                <div>
                                    ...
                                    <p class="text-slate-400 text-[10px] font-black uppercase tracking-widest italic underline decoration-slate-100 underline-offset-4">Click for Details →</p>
                                </div>
                            </div>`;
                        }}
                    }});
...
<body class="bg-white flex flex-col min-h-screen text-slate-900">
        ...
        <div class="modal-content shadow-2xl animate-in zoom-in duration-300">
            <div class="grid md:grid-cols-2">
                ...
                <div class="p-10 flex flex-col justify-center text-left">
                    ...
                    <button onclick="document.getElementById('modal').style.display='none'; document.body.style.overflow='auto';" class="text-xs font-black uppercase tracking-widest opacity-30 mt-4 underline no-underline">Close</button>
                </div>
            </div>
        </div>
    </div>
    ...
    d_section = f"""<section id="inventory" class="py-32 px-6 max-w-[1440px] mx-auto text-center border-b"><h2 class="section-title mb-20 uppercase tracking-tighter" style="color:var(--p)">Verified Packages</h2><div id="live-data-container" class="grid grid-cols-1 md:grid-cols-4 gap-10 text-left"><p class="p-20 text-center text-slate-400 font-bold animate-pulse uppercase tracking-widest">Connecting to Data Hub...</p></div></section>""" if sheet_url else ""
    ...
    <section class="bg-slate-50 py-32 px-6 border-y text-left">
        <div class="max-w-[1440px] mx-auto grid md:grid-cols-2 gap-24 items-center">
            ...
            <div>
                ...
                <a href="about.html" class="btn-p text-xs tracking-widest uppercase no-underline">View Full Story</a>
            </div>
        </div>
    </section>
    ...
    st.success("💎 TITAN SUPREME v11.2 FINAL DEPLOYED. Fixed all bugs and restored full UI.")
    st.download_button("📥 DOWNLOAD PLATINUM PACKAGE", z_b.getvalue(), f"{biz_name.lower().replace(' ', '_')}_v11_2.zip")
